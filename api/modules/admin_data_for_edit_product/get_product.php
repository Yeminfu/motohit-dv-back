...
function get_product($product_name)
{
    ...
    $images = $mysqli->query("SELECT id, name FROM `media` WHERE essense_id = $product_id ")->fetch_all(MYSQLI_ASSOC);
    ...
    foreach ($attributes as $key => $attribute) {
        ...
        $qs_get_value = "SELECT value_name, id as value_id FROM attributes_values WHERE id IN
        (SELECT attribute_value FROM attr_prod_relation WHERE product = $product_id AND attribute = $attribute_id)
    ";
        $attribute_value = $mysqli->query($qs_get_value)->fetch_assoc();
